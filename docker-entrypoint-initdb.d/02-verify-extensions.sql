...
--
-- Runs after 00 (age) and 01 (vector, vchord, pg_search, pg_trgm, libversion)
...
-- caught failure (see docs/roadmap and ADR-0005). No-op once all six exist.
DO $$
DECLARE
  expected text[] := ARRAY['age', 'vector', 'vchord', 'pg_search', 'pg_trgm', 'libversion'];
  missing  text[];
BEGIN
