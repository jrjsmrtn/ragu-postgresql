...
[Tiger Data pg_textsearch](https://github.com/timescale/pg_textsearch) on top of
the official [Apache AGE](https://age.apache.org/) image so one PostgreSQL can back:
...
- **Full-text / hybrid retrieval** — BM25 ranking via `pg_textsearch`,
...
| Full-text  | `pg_textsearch` | 1.3.1 (`v1.3.1`)  | built from source               |
...
runs natively on Apple Silicon. Apache AGE and pg_textsearch both require
preloading, so this image overrides the base `CMD` to
`postgres -c shared_preload_libraries=age,pg_textsearch`.
...
- `01-create-extensions-rag.sql` (this image) → `vector`,
  `pg_textsearch`, `pg_trgm`, `libversion`
...
> vector, pg_textsearch, pg_trgm, libversion`) run **only on first init of an empty
...
- **Build-time integrity** — the base image is pinned by digest and every
  source-built extension (pgvector, pg_textsearch, libversion) is pinned by git
  tag. No `.deb` is fetched at build time (VectorChord, the only one, was removed
  — [ADR-0008](docs/adr/0008-remove-vectorchord.md)).
...
The **built image is a fully permissive aggregate**: every bundled component —
PostgreSQL, Apache AGE, pgvector, pg_trgm, `pg_textsearch`, and `libversion` — is
permissively licensed (`Apache-2.0 AND PostgreSQL AND MIT`). There is **no
copyleft floor**: the only copyleft component, VectorChord, was removed
([ADR-0008](docs/adr/0008-remove-vectorchord.md)), so no AGPL source-offer or
Elastic License managed-service restriction applies. Redistributing the image or
offering it as a network service carries only attribution obligations. See
[`LICENSING.md`](LICENSING.md) and
[ADR-0008](docs/adr/0008-remove-vectorchord.md) for the per-component breakdown.
